      ...
      context "on #{os}" do
        ...
        context 'with defalt params' do
          ...
          if os_facts[:os][:release][:major].to_i >= 10
            it { is_expected.to create_sssd__config__entry('puppet_service_nss').without_content(%r{^reconnection_retries}) }
          else
            it { is_expected.to create_sssd__config__entry('puppet_service_nss').with_content(%r{^reconnection_retries = 3$}) }
          end
        end
